...
import { Activity, BarChart3, Sliders, Terminal as TermIcon, ShieldAlert, ArrowLeft, LogOut } from 'lucide-react';
...
export default function Admin() {
  ...
  const handleLogout = () => {
    playClick();
    sessionStorage.removeItem('alterscore_admin_authorized');
  ...
  const modelStats = [
    { name: 'Stacking Ensemble', type: 'ensemble', auc: 0.812, pr: 0.86, ks: 0.47, brier: 0.14, ece: 0.03, selected: true },
    { name: 'TabNet PyTorch', type: 'neural', auc: 0.791, pr: 0.82, ks: 0.44, brier: 0.15, ece: 0.05 },
    { name: 'XGBoost Baseline', type: 'classical', auc: 0.775, pr: 0.80, ks: 0.41, brier: 0.17, ece: 0.06 },
    { name: 'Logistic Regression', type: 'classical', auc: 0.720, pr: 0.74, ks: 0.35, brier: 0.18, ece: 0.04 }
  ];
  ...
  if (!isAuthorized) {
    return (
      <div className="terminal-gate-container">
        <div className={`terminal-gate-card ${shake ? 'shake' : ''}`}>
          ...
          <div className="terminal-gate-body font-mono">
            <p className="status-line">LOGGED HOST: WORKSTATION-01</p>
            <p className="status-line">SYSTEM STATUS: ACTIVE</p>
            <p className="status-line text-warning">PRIVILEGES: RESTRICTED // ROOT REQUIRED</p>
            <div className="passcode-field-wrapper">
              <form onSubmit={handleAuthSubmit}>
                <label htmlFor="gate-passcode">ENTER OPERATOR PASSCODE:</label>
            ...
            {authError && (
              <p className="access-denied-label text-danger">*** ACCESS DENIED: INVALID PRIVILEGES ***</p>
            )}
    ...
    <div className="dashboard-layout">
      ...
      <aside className="dashboard-sidebar">
        <div className="sidebar-title">
          ...
          <span>AlterScore Operator HUD</span>
        </div>
        ...
        <div className="sidebar-footer">
          ...
          <p style={{ opacity: 0.4, marginTop: '12px' }}>v2.0.0-PROD</p>
        </div>
      </aside>
