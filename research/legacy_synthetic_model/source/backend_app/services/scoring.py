...
class ScoringService:
    ...
    def _run_scoring_pipeline(
        ...
        assembled = assemble_request_features(score_request)
        ...
        base_credit_score = int(score_mapping_details["final_credit_score"])
        text_quality = assembled.text_quality
        credit_score = int(
            np.clip(
                base_credit_score + text_quality.score_adjustment_points,
                SCORE_MIN,
                SCORE_MAX,
            )
        )
        score_mapping_details.update(
            {
                "base_credit_score": base_credit_score,
                "text_quality_adjustment_points": text_quality.score_adjustment_points,
        ...
        counterfactual_actions = _build_counterfactual_actions(
            ...
            candidate_score_mapper=lambda probability: int(
                np.clip(
                    probability_to_score(probability, score_mapping_config)
                    + text_quality.score_adjustment_points,
                    SCORE_MIN,
                    SCORE_MAX,
                )
            ),
        ...
        response = ScoreResponse.model_validate(
            {
                ...
                "text_quality": text_quality.as_dict(),
        ...
        debug_trace = {
            "score_policy": {
                ...
                "browser_telemetry_affects_score": False,
                "opaque_text_embeddings_affect_score": False,
            },
            "text_quality_adjustment": {
                **text_quality.as_dict(),
                "base_credit_score": base_credit_score,
                "final_credit_score": credit_score,
            },
            ...
            "raw_behavioral_diagnostics": _to_jsonable(
...
def _build_preprocessor_debug(preprocessor: Any, feature_frame: Any) -> dict[str, Any]:
    ...
    numeric_features = transformer_columns.get("num", [])
    categorical_features = transformer_columns.get("cat", [])
    ...
    categorical_imputed: np.ndarray = np.empty((1, 0), dtype=object)
    categorical_encoded: np.ndarray = np.empty((1, 0), dtype=float)
    categorical_encoder_categories: dict[str, list[Any]] = {}
    ...
    return {
        ...
        "categorical_encoder_categories": categorical_encoder_categories,
...
def _build_improvement_tips(feature_row: dict[str, Any]) -> list[ImprovementTip]:
    ...
    return [
        ImprovementTip(
            ...
            title="Keep building planning habits",
            body="Practice concrete planning and follow-through in future assessment scenarios.",
        )
    ]
...
def _calculate_governance_multiplier(
    ...
    """Return the neutral policy used by offline governance reporting."""
    del feature_row
    return 1.0, []
