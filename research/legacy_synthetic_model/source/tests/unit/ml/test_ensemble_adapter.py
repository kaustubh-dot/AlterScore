...
def test_build_ensemble_meta_features_triggers_tabnet_disagreement_mitigation() -> None:
    ...
    assert meta_features.mitigation_debug["raw_tabnet_probability"][0] == pytest.approx(
        0.03
    )
    ...
    assert (
        meta_features.mitigation_debug["adjusted_tabnet_probability"][0]
        < meta_features.mitigation_debug["peer_mean_probability"][0]
    )


def test_build_ensemble_meta_features_does_not_trigger_when_tabnet_agrees_with_peers() -> (
    None
):
    stacking_model = MockStackingModel()
    base_models = {
    ...
    assert meta_features.mitigation_debug["adjusted_tabnet_probability"][
        0
    ] == pytest.approx(meta_features.mitigation_debug["raw_tabnet_probability"][0])
