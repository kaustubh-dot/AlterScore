...
def test_numeric_feature_count_is_answer_only() -> None:
    assert len(NUMERIC_FEATURES) == 11


def test_categorical_feature_count_is_zero() -> None:
    assert CATEGORICAL_FEATURES == []


def test_total_model_input_count_is_answer_only() -> None:
    assert ALL_MODEL_FEATURES == NUMERIC_FEATURES + CATEGORICAL_FEATURES
    assert len(ALL_MODEL_FEATURES) == 11
    assert len(set(ALL_MODEL_FEATURES)) == 11
