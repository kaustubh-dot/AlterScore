...
def test_score_mapping_full_range_reachable() -> None:
    # Strong model probabilities should be represented near the high end.
    top_score = probability_to_score(0.96)
    assert top_score >= 750, f"p=0.96 -> {top_score}, upper range unreachable"

    # Weak model probabilities should be represented in the lower range.
    weak_score = probability_to_score(0.30)
    assert weak_score < 550, f"p=0.30 -> {weak_score}, expected lower range"
