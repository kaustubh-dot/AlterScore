...
def test_runtime_feature_assembly_keeps_persisted_text_pca_score_inert(
    tmp_path,
) -> None:
    _prepare_runtime_bundle(tmp_path, include_text_pca=True)
    payload = _load_valid_score_payload()
    text_pca = joblib.load(tmp_path / "models" / "preprocessors" / "text_pca.pkl")

    without_pca = assemble_request_features(payload)
    with_pca = assemble_request_features(
        payload,
        text_pca=text_pca,
        require_text_pca=True,
    )

    assert with_pca.feature_row == without_pca.feature_row
    assert with_pca.text_quality == without_pca.text_quality


def test_runtime_feature_assembly_never_emits_semantic_text_features() -> (
    None
):
    ...
    assert "text_semantic_dim1" not in assembled.feature_row
    assert "text_semantic_dim2" not in assembled.feature_row


def test_artifact_loader_succeeds_when_runtime_bundle_omits_text_pca(
    tmp_path,
) -> None:
    bundle = _prepare_runtime_bundle(tmp_path, include_text_pca=False)
    response = score_request_with_bundle(_load_valid_score_payload(), bundle)

    assert bundle.report.scoring_ready is True
    assert "text_pca" not in bundle.report.artifacts_loaded
