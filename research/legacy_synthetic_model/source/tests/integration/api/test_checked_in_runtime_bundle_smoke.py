...
def _checked_in_production_manifest() -> dict:
    return json.loads(
        (REPO_ROOT / "models" / "registry" / "production_manifest.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_checked_in_bundle_loader_validates_real_runtime_artifacts() -> None:
    ...
    manifest = _checked_in_production_manifest()
...
def test_checked_in_bundle_health_endpoint_reports_validated_optional_status() -> None:
    ...
    assert payload.promotion_gate.policy_version == "promotion_gate_policy_v2"
...
def test_checked_in_bundle_global_importance_endpoint_serves_saved_payload() -> None:
    ...
    assert len(payload.items) == len(ALL_MODEL_FEATURES)
...
def test_checked_in_bundle_score_endpoint_appends_to_runtime_log_path() -> None:
    ...
    assert entries[0]["manifest_version"] == manifest["manifest_version"]
    assert entries[0]["model_version"] == manifest["model_version"]
