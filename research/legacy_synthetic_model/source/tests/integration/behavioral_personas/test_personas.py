...
def test_impulsive_applicant_is_penalized(runtime_artifacts):
    ...
    res = score_request_with_bundle(req, runtime_artifacts)
    thoughtful = score_request_with_bundle(
        _build_request(PRESETS["thoughtful"]), runtime_artifacts
    )
    assert res.credit_score < thoughtful.credit_score
    assert res.repayment_probability < thoughtful.repayment_probability


def test_manipulated_answers_do_not_outrank_thoughtful_answers(runtime_artifacts):
    req = _build_request(PRESETS["manipulated"])
    res = score_request_with_bundle(req, runtime_artifacts)
    # The profiles differ in declared scenario answers. Browser diagnostics are
    # not part of either score.
    ...
    ), "Manipulated answers must not outrank thoughtful answers."
    assert (
        res.repayment_probability < res_thoughtful.repayment_probability
    ), "Answer-derived model probability must remain ordered."


def test_average_applicant_preserves_relative_persona_order(runtime_artifacts):
    ...
    assert res.credit_score < res_thoughtful.credit_score
    assert (
        abs(res.repayment_probability - res_thoughtful.repayment_probability) < 0.10
