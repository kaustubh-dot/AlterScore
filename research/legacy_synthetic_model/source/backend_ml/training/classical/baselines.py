...
class SyntheticHeuristicBaseline:
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        ...
        heuristic = (
            ...
            + 0.10 * X["resilience_score"].to_numpy(dtype=float)
        )
    ...
    def _assert_required_columns(self, X: pd.DataFrame) -> None:
        ...
        if missing_columns:
            raise ValueError(
                f"SyntheticHeuristicBaseline is missing required columns: {missing_columns}"
            )
...
def train_baselines(
    ...
    *,
    ...
    heuristic_model = SyntheticHeuristicBaseline(random_state=random_state).fit(
        prepared.train.X
    )
    ...
    comparison_metrics = {
        ...
        "synthetic_heuristic": compute_binary_classification_metrics(
            prepared.test.y.to_numpy(dtype=int),
            heuristic_model.predict_proba(prepared.test.X)[:, 1],
            model_name="synthetic_heuristic",
            model_type="baseline",
            split="test_months_11_12",
        ),
    }
    reference_auc = comparison_metrics["synthetic_heuristic"]["auc_roc"]
    baseline_metrics = [
...
def _build_baseline_comparison_item(
    ...
    *,
    ...
    return {
        ...
        "lift_vs_synthetic_heuristic": round(metrics["auc_roc"] - reference_auc, 4),
    }
