...
def test_train_baselines_persists_finite_evaluation_and_percentile_artifacts(
    ...
    assert confusion_matrix["tp"] + confusion_matrix["fp"] + confusion_matrix["fn"] + confusion_matrix["tn"] == metrics_payload["split_row_counts"]["test"]
    ...
    assert sum(bucket["count"] for bucket in population_percentiles["score_histogram"]) == 1_200
