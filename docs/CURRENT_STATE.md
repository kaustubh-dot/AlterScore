...
- Calibrated stacking ensemble training exists at `backend/ml/training/ensemble/train_stacking.py`, with CLI entrypoint `scripts/training/train_stacking.py`. The module accepts a `StackingInputs` dataclass (or re-trains all 6 base models automatically) and fits a `LogisticRegression` meta-learner on the validation-month probability matrix, then wraps it in `CalibratedClassifierCV(method='isotonic', cv='prefit')`. Artifacts are a `.pkl` (joblib) plus a `calibrated_stacking_config.json` sidecar; `load_stacking_model` and `predict_stacking_proba` are the inference-time counterparts. The `backend/ml/training/ensemble/__init__.py` package file exists.
