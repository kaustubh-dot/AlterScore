...
- Report-backed analytics cover the full documented route surface. Both neural training modules (TabNet, residual MLP) are complete but no neural artifacts are checked into `models/` yet (offline-only until stacking promotes a calibrated ensemble). A promoted calibrated production ensemble, interactive frontend tests, and broader ML validation are still pending.
...
1. Implement the stacking ensemble training module (`backend/ml/training/ensemble/train_stacking.py`) that takes validation-split probability arrays from all 6 base models (logistic, RF, XGBoost, LightGBM, TabNet, MLP) and fits a logistic meta-learner, followed by isotonic calibration on months 9-10.
2. After a calibrated ensemble artifact exists, refresh the SHAP and DICE explainability artifacts against the new production candidate and update the manifest.
