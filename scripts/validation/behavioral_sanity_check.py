...
from backend.ml.preprocessing.feature_registry import ALL_MODEL_FEATURES, PROTECTED_FEATURES, ACTIONABLE_FEATURES
...
def load_base_request() -> dict:
    ...
    if not fixture_path.exists():
        raise FileNotFoundError(f"Base score request fixture not found at {fixture_path}")
...
def perturb_request(base: dict, answers_updates: dict = None, behavioral_updates: dict = None) -> dict:
...
def check_governance_and_loading(bundle) -> None:
    ...
    print(f"  [+] OK: Model name: '{report.runtime_model_name}', type: '{report.runtime_model_type}'")
...
def check_protected_features() -> None:
    ...
    for pf in PROTECTED_FEATURES:
        if pf in ALL_MODEL_FEATURES:
            raise ValueError(f"Protected feature '{pf}' was found in ALL_MODEL_FEATURES!")
...
def check_score_semantics_and_risk_bands(service, base_request) -> None:
    ...
    if risk_band != expected_band:
        raise ValueError(f"Risk band mismatch. Score: {score}, Got: {risk_band}, Expected: {expected_band}")
        
    eligibility = response.loan_eligibility
    if eligibility.band != risk_band:
        raise ValueError(f"Loan eligibility band '{eligibility.band}' doesn't match risk band '{risk_band}'")
    ...
    print(f"  [+] OK: Loan eligibility bounds correctly aligned: {eligibility.amount_min} - {eligibility.amount_max}")
...
def check_monotonicity(service, base_request) -> None:
    ...
    for val in time_vals:
        req = perturb_request(base_request, behavioral_updates={"avg_response_time_ms": val})
        res = service.score_request(req)
        scores.append(res.credit_score)
        probs.append(res.repayment_probability)
    ...
    for val in scroll_vals:
        req = perturb_request(base_request, behavioral_updates={"scroll_hesitation_score": val})
        res = service.score_request(req)
        scores.append(res.credit_score)
        probs.append(res.repayment_probability)
    ...
    for val in change_vals:
        req = perturb_request(base_request, behavioral_updates={"answer_change_rate": val})
        res = service.score_request(req)
        scores.append(res.credit_score)
        probs.append(res.repayment_probability)
    ...
    for val in cons_vals:
        req = perturb_request(base_request, answers_updates={"conscientiousness_q1": val})
        res = service.score_request(req)
        scores.append(res.credit_score)
        probs.append(res.repayment_probability)
    ...
    print("  [+] OK: Monotonicity verified successfully for numeric and psychometric features.")
...
def check_explanations(service, base_request) -> None:
    ...
    for item in response.explanation:
        ...
        expected_display = FEATURE_DISPLAY_NAMES.get(feature, feature.replace("_", " ").title())
        if display_name != expected_display:
            raise ValueError(f"Display name mismatch for '{feature}': Got '{display_name}', Expected '{expected_display}'")
            
        # Verify direction matches SHAP sign
        if direction == "positive" and shap_value < 0.0:
            raise ValueError(f"Direction is 'positive' but SHAP value is negative ({shap_value}) for {feature}")
        if direction == "negative" and shap_value >= 0.0:
            raise ValueError(f"Direction is 'negative' but SHAP value is positive/zero ({shap_value}) for {feature}")
    ...
    print("  [+] OK: Explanation directions, display names, and plain language verified.")
...
def check_counterfactual_realism(service, base_request) -> None:
    ...
    if score < 850:
        if not actions:
            raise ValueError(f"No counterfactual actions generated for sub-maximal score {score}")
        ...
        for action in actions:
            ...
            if feature not in ACTIONABLE_FEATURES:
                raise ValueError(f"Counterfactual proposed on non-actionable feature: {feature}")
            ...
            if policy:
                ...
                if direction == "increase" and suggested_value <= current_value:
                    raise ValueError(f"CF suggests decreasing/equal value ({suggested_value} <= {current_value}) for positive feature {feature}")
                if direction == "decrease" and suggested_value >= current_value:
                    raise ValueError(f"CF suggests increasing/equal value ({suggested_value} >= {current_value}) for negative feature {feature}")
                    
            if gain < 0:
                raise ValueError(f"Negative score gain {gain} for feature {feature}")
                
        print(f"  [+] OK: Generated {len(actions)} realistic actions on actionable features.")
    ...
    perfect_req = perturb_request(
        base_request,
        answers_updates={
            "numeracy_q1": 6600, "numeracy_q2": 1120.0, "numeracy_q3": 14400.0,
            "financial_literacy_q1": 1, "financial_literacy_q2": 1,
            "conscientiousness_q1": 5,
            "CRT_q1": 5.0, "CRT_q2": 5.0, "CRT_q3": 47,
            "future_orient_q1": 1, "future_orient_q2": 1, "future_orient_q3": 5,
            "risk_q1": 0, "risk_q2": 0,
            "locus_q1": 0, "locus_q2": 0, "locus_q3": 5,
            "social_capital_q1": 3, "social_capital_q2": 0, "social_capital_q3": 0,
            "resilience_q1": 5, "resilience_q2": 5, "resilience_q3": 0,
            "loss_aversion_q1": 2,
            "honesty_trap_q1": 1, "honesty_trap_q2": 1,
            "future_orient_repeat": 1, "locus_repeat": 0,
            "reciprocity_q1": 5, "reciprocity_q2": 0,
    ...
    if perfect_res.credit_score != 850:
        print(f"  [!] Warning: Perfect profile score is {perfect_res.credit_score}, not 850. Continuing boundary checks...")
    ...
    print(f"  [+] OK: Boundary score: {perfect_res.credit_score}, counterfactuals: {len(perfect_res.counterfactual_actions)} items")
...
def check_extreme_profiles(service, base_request) -> None:
    ...
    poor_req = perturb_request(
        base_request,
        answers_updates={
            "numeracy_q1": 0, "numeracy_q2": 0.0, "numeracy_q3": 0.0,
            "financial_literacy_q1": 0, "financial_literacy_q2": 0,
            "conscientiousness_q1": 1,
            "CRT_q1": 0.0, "CRT_q2": 0.0, "CRT_q3": 1,
            "future_orient_q1": 0, "future_orient_q2": 0, "future_orient_q3": 1,
            "risk_q1": 1, "risk_q2": 0,
            "locus_q1": 2, "locus_q2": 2, "locus_q3": 1,
            "social_capital_q1": 0, "social_capital_q2": 2, "social_capital_q3": 2,
            "resilience_q1": 1, "resilience_q2": 1, "resilience_q3": 3,
            "loss_aversion_q1": 0,
            "honesty_trap_q1": 5, "honesty_trap_q2": 5,
            "future_orient_repeat": 0, "locus_repeat": 2,
            "reciprocity_q1": 1, "reciprocity_q2": 2,
    ...
    print(f"  - Perfect Profile -> Score: {perfect_res.credit_score}, Band: {perfect_res.risk_band}, Prob: {perfect_res.repayment_probability:.4f}")
    print(f"  - Poor Profile    -> Score: {poor_res.credit_score}, Band: {poor_res.risk_band}, Prob: {poor_res.repayment_probability:.4f}")
    ...
    if perfect_res.risk_band not in ["excellent", "good"]:
        raise ValueError(f"Perfect profile risk band should be 'excellent' or 'good', got '{perfect_res.risk_band}'")
        
    if poor_res.risk_band != "poor":
        raise ValueError(f"Poor profile risk band should be 'poor', got '{poor_res.risk_band}'")
...
def check_golden_profiles(service) -> None:
    ...
    for entry in profiles:
        ...
        if (eligibility.amount_min != exp["loan_eligibility"]["amount_min"] or
                eligibility.amount_max != exp["loan_eligibility"]["amount_max"]):
            raise ValueError(
    ...
    print(f"  [+] OK: All {len(profiles)} golden reference profiles scored and matched target specifications.")
...
def check_malformed_and_boundary_payloads(service, base_request) -> None:
    ...
    try:
        ...
        raise ValueError("Failed to raise ValidationError for missing 'answers' component")
    except ValidationError:
        pass  # Expected Pydantic validation error
    except Exception as e:
        raise ValueError(f"Unexpected exception for missing answers: {type(e).__name__}: {e}")

    # 2. Invalid answer value type/out-of-bounds (Pydantic guard)
    print("  - Testing out-of-bounds input (conscientiousness_q1=10)...")
    try:
        bad_req = perturb_request(base_request, answers_updates={"conscientiousness_q1": 10})
        service.score_request(bad_req)
        raise ValueError("Failed to raise ValidationError for conscientiousness_q1=10")
    except ValidationError:
        pass  # Expected Pydantic validation error
    except Exception as e:
        raise ValueError(f"Unexpected exception for invalid conscientiousness_q1: {type(e).__name__}: {e}")

    # 3. Empty text q27_resilience_text (ensure robust fallback)
    print("  - Testing empty resilience text fallback...")
    empty_txt_req = perturb_request(base_request, answers_updates={"q27_resilience_text": ""})
    ...
    fast_time_req = perturb_request(base_request, behavioral_updates={"avg_response_time_ms": 100.0})
    ...
    slow_time_req = perturb_request(base_request, behavioral_updates={"avg_response_time_ms": 120000.0})
    ...
    try:
        bad_req = perturb_request(base_request, behavioral_updates={"device_type": "smartwatch"})
        service.score_request(bad_req)
        raise ValueError("Failed to raise ValidationError for invalid device_type 'smartwatch'")
    except ValidationError:
        pass  # Expected
    except Exception as e:
        raise ValueError(f"Unexpected exception for invalid device_type: {type(e).__name__}: {e}")

    print("  [+] OK: Schema validation and boundary values behave correctly and robustly.")
...
def main() -> int:
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("[+] SUCCESS: All behavioral, golden regression, and schema checks passed!")
        print("=" * 60)
        return 0
